...
<%
	...
	//글 조회
	ArticleDTO dto = art.selectArticle(no);
	
	//글 조회 카운트 업데이트 
	art.updateHitCount(no);
	
	//댓글 조회
	List<ArticleDTO> comments= art.selectCommnets(no);
...
<main>
    <section class="view">
        ...
        <div>
            ...
            <a href="/jboard1/proc/deleteProc.jsp?no=<%= dto.getNo() %>&pg=<%=pg %>" class="btnDelete">삭제</a>
            <a href="/jboard1/modify.jsp?no=<%=dto.getNo() %>&pg=<%=pg %>" class="btnModify">수정</a>
            <% } %>
